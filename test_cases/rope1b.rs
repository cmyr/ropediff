...
pub type ScopeDelta = Delta<RopeInfo>;

/// An element in a `ScopeDelta`.
pub type ScopeDeltaElement = DeltaElement<RopeInfo>;
...
impl<'de> Deserialize<'de> for Delta<RopeInfo> {
        ...
        where D: Deserializer<'de>,
    {
        ...
        #[derive(Serialize, Deserialize)]
        struct ScopeDelta_ {
            els: Vec<ScopeDeltaElement_>,
            base_len: usize
        }

        impl From<ScopeDeltaElement_> for DeltaElement<RopeInfo> {
            fn from(elem: ScopeDeltaElement_) -> DeltaElement<RopeInfo> {
                match elem {
                    ScopeDeltaElement_::Copy(start, end) =>
                        DeltaElement::Copy(start, end),
                    ScopeDeltaElement_::Insert(s) =>
                        DeltaElement::Insert(Rope::from(s)),
                }
            }
        }

        impl From<ScopeDelta_> for Delta<RopeInfo> {
            fn from(mut delta: ScopeDelta_) -> Delta<RopeInfo> {
                Delta {
                    els: delta.els.drain(..)
        ...
        let d = ScopeDelta_::deserialize(deserializer)?;
        Ok(Delta::from(d))
    }
}
